        ...
        stage("Agent: NodeJS") {
          ...
          steps {            
            script {
              ...
              gitTagName = sh (
                returnStdout: true,
                script: "git tag -l '*rc*' | cat | tail -n 1 | xargs | tr -s ' '"
              )
              println gitTagName
              ...
              def gitMasterHead = sh (
                returnStdout: true,
                script: "git log | cat | head -n 1 | awk -F' ' '{print \$2}' | xargs | tr -s ' '"
              )
              println gitMasterHead
              def gitTagHead = sh (
                returnStdout: true,
                script: "git log '${gitTagName}' | cat | head -n 1 | awk -F' ' '{print \$2}' | xargs | tr -s ' '"
              )
              println gitTagHead
            ...
            script {
              try {
                ...
                containerPort.each { portValue ->
                  exposedPort = sh (
                    returnStdout: true,
                    script: "docker ps -a -f 'name=${params.KUBE_DEV_NAMESPACE}-${params.DOCKER_IMAGE_NAME}-${params.DOCKER_IMAGE_TAG}' --format '{{.Ports}}' | awk '{for(i=1;i<=NF;i++){tmp=match(\$i,/${portValue}/);if(tmp){print \$i}}}' | cut -d'-' -f1 | cut -d':' -f2 | xargs"
                  )
